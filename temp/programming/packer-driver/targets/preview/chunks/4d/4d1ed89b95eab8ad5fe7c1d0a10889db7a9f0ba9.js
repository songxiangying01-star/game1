    ...
    execute: function () {
      ...
      _export("default", UiEdColumnCount = (_dec = ccclass('UiEdColumnCount'), _dec2 = menu('cwg/UiEdColumnCount'), _dec3 = property(Node), _dec4 = property(Label), _dec(_class = _dec2(_class = (_class2 = class UiEdColumnCount extends Component {
        ...
        reset(funland) {
          if (funland) {
            ...
            funland.glasses.forEach(item => {
              var key = item.position.x;
              var items = columnMap.get(key) || [];
              items.push(item);
        ...
        updateCountDisplay() {
          ...
          var width = 744 / this.count;
          var sortedEntries = Array.from(this.columnMap.entries()).sort((a, b) => a[0] - b[0]);
          var maxCnt = sortedEntries.length;
          this.columnCtrlsContent.children.forEach((item, index) => {
            var colument = item.getComponent(_crd && UiEdColumnCtrl === void 0 ? (_reportPossibleCrUseOfUiEdColumnCtrl({
              error: Error()
...
//# sourceMappingURL=4d1ed89b95eab8ad5fe7c1d0a10889db7a9f0ba9.js.map