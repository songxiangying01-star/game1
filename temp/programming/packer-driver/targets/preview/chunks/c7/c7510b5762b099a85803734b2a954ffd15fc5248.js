    ...
    execute: function () {
      ...
      _export("VwExchange", VwExchange = (_dec = ccclass('VwExchange'), _dec2 = menu('cwg/VwExchange'), _dec3 = property(Node), _dec4 = property(Prefab), _dec5 = property(_crd && VwFunland === void 0 ? (_reportPossibleCrUseOfVwFunland({
        ...
        show() {
          ...
          var candidates = this.funlandView.glasses.filter(glass => !glass.isAd() && !glass.isEmpty && !glass.isSealed() && !glass.isAllHide());

          if (candidates.length > 0) {
            // 从符合条件的瓶子中随机选择一个
            var randomIndex = Math.floor(Math.random() * candidates.length);
            var selectedGlass = candidates[randomIndex]; // 直接执行交换逻辑，不显示任何特效
...
//# sourceMappingURL=c7510b5762b099a85803734b2a954ffd15fc5248.js.map