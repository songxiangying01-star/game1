    ...
    execute: function () {
      ...
      _export("VwEdPlay", VwEdPlay = (_dec = ccclass('VwEdPlay'), _dec2 = menu('cwg/VwEdPlay'), _dec3 = property({
        ...
        async restartLevel() {
          ...
          this.ui.reset(this.gameState.info, this.funland); // 开始游戏
        ...
        exportLevelData() {
          ...
          this.columnsNode.children.forEach(node => {
            ...
            const columnController = node.getComponent(_crd && UiEdColumn === void 0 ? (_reportPossibleCrUseOfUiEdColumn({
              error: Error()
            }), UiEdColumn) : UiEdColumn); // 收集该列中的所有杯子数据

            columnController.glassesNode.children.forEach(glassNode => {
              const glassController = glassNode.getComponent(_crd && EdGlass === void 0 ? (_reportPossibleCrUseOfEdGlass({
                error: Error()
              }), EdGlass) : EdGlass);
              const glassData = glassController.info; // 记录杯子的位置信息
...
//# sourceMappingURL=e06ebcd69fcf9ab17365f80daa8b2e66a4ef612f.js.map