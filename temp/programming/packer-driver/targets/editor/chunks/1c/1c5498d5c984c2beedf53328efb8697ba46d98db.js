  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", GlassFlowing = (_dec = ccclass('GlassFlowing'), _dec2 = menu('cwg/GlassFlowing'), _dec3 = property(Node), _dec4 = property(_crd && SoundComp === void 0 ? (_reportPossibleCrUseOfSoundComp({
         ...
         */
        async playWaterUpAnim(index) {
          const waterNode = this.watersNode.children[index]; // 【关键修复】增加安全检查：如果水节点不存在（例如索引越界），直接返回，避免报错
          ...
          const waterTransform = waterNode.getComponent(UITransform); // 【安全检查】确保组件存在
          ...
          const flowingTf = this.flowingNode.getComponent(UITransform); // 速度加快：0.15s
        ...
        async playPutOnCapAnim() {
          if (!this.capNode) return;
          this.capNode.active = true;
          this.capNode.y = 276; // 盖盖子速度：0.2s

          await (_crd && Toolkit === void 0 ? (_reportPossibleCrUseOfToolkit({
            error: Error()
          }), Toolkit) : Toolkit).waitForTween(tween(this.capNode).to(0.2, {
            y: 216
          }));
        }
        /**
...
//# sourceMappingURL=1c5498d5c984c2beedf53328efb8697ba46d98db.js.map